            ...
            { 
              loading ? (
                ...
                data.map((product,index)=>{
                  return (
                    <Link to={"/product/"+product?._id} className='min-w-[280px] max-w-[320px] bg-white rounded-xl shadow-md p-4 flex flex-col items-center hover:shadow-xl hover:scale-[1.03] transition-all duration-300 group' key={index}>
                      <div className='bg-slate-100 p-4 h-44 w-full flex justify-center items-center rounded-lg mb-4 overflow-hidden'>
                        <img src={product.productImage[0]} className='h-32 object-contain group-hover:scale-110 transition-transform duration-300 bg-transparent mix-blend-multiply'/>
                      </div>
                      <div className='flex flex-col justify-between w-full flex-1'>
                        ...
                        <button className='bg-red-600 text-white rounded-full px-6 py-2 w-full hover:bg-red-700 transition-all duration-200 font-semibold shadow' onClick={(e)=>handleAddToCart(e,product?._id)}>
                          Add to Cart
                        </button>
                      </div>
                    </Link>
